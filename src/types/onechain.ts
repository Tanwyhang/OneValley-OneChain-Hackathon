...
// === Extended Frontend Item Types ===

export interface ExtendedFrontendItem extends FrontendItem {
  ...
  formatted_rarity?: string;
  rarity_color?: string;
  formatted_item_type?: string;

  // Blockchain display fields
  formatted_minter?: string;
  formatted_owners?: string[];
  formatted_timestamp?: string;
  transaction_history?: TransactionHistoryEntry[];
}

export interface TransactionHistoryEntry {
  type: 'minted' | 'transferred' | 'listed' | 'sold' | 'cancelled';
  timestamp: number;
  from?: Address;
  to?: Address;
  transaction_hash: string;
...
export interface MarketplaceFilters {
  ...
  rarity?: RarityLevel;
  search?: string;
  sortBy?: 'price' | 'rarity' | 'listed_at' | 'item_name';
  sortOrder?: 'asc' | 'desc';
}

export interface MarketplaceTab {
  id: 'browse' | 'my_kiosk' | 'sales_history';
  label: string;
  icon?: string;
}

export interface KioskListingAction {
  type: 'list' | 'unlist' | 'edit_price';
  itemId?: string;
  listingId?: string;
  price?: number;
}
...
export interface MarketplaceTransaction {
  id: string;
  type: 'listing' | 'purchase' | 'cancellation';
...
export interface MarketplaceUIState {
  // Current view
  activeTab: MarketplaceTab['id'];
  selectedListing?: MarketplaceListing;
  selectedItem?: FrontendItem;

  // Data
  marketplaceListings: MarketplaceListing[];
  playerKiosk: PlayerKiosk | null;
  playerInventory: FrontendItem[];
  ...
  // Transaction state
  pendingTransactions: MarketplaceTransaction[];
  transactionHistory: MarketplaceTransaction[];

  // Kiosk management
  listingMode: 'create' | 'edit' | null;
  listingItem?: FrontendItem;
  listingPrice?: number;
...
export interface MarketplaceListedEvent {
  type: 'listed';
  listing_id: string;
  item_id: string;
  seller: Address;
...
export interface MarketplaceSoldEvent {
  type: 'sold';
  listing_id: string;
  item_id: string;
  seller: Address;
...
export interface MarketplaceCancelledEvent {
  type: 'cancelled';
  listing_id: string;
  item_id: string;
  seller: Address;
...
export type MarketplaceEvent = MarketplaceListedEvent | MarketplaceSoldEvent | MarketplaceCancelledEvent;